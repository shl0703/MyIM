...
//			System.out.println("解密后：" + DES.decrypt(result, password));
		...
		/*	String sdddd=new BASE64Encoder().encode(DES.encrypt(str,password));
            System.out.println(sdddd);


			String aaa=DES.decrypt(new BASE64Decoder().decodeBuffer(sdddd), password);
    ...
    public static String encrypt(String datasource, String password) {
        try {
            ...
            // 现在，获取数据并加密
            // 正式执行加密操作
            return Base64.encodeToString(cipher.doFinal(datasource.getBytes()), 0);
